...
class AdapterDB(DB):
    ...
    def get_users(self):
        return list(map(lambda x: x[0], self.exe("SELECT id FROM users").fetchall()))
    ...
    def get_all_help_requests(self):
        return list(map(lambda x: data_to_dict("help_requests", x),
                        self.exe("SELECT * FROM help_requests ORDER BY time_requested DESC").fetchall()))
    ...
    def get_events_by_day(self, year, month, day) -> list:
        ...
        return self.get_events_by_time_stamp(day_timestamp)
